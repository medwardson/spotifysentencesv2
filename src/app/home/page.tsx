      ...
      {id === undefined ? 
        ...
        <form id="song-form" className="flex flex-col items-center w-6/12 mb-12" onSubmit={(e) => {
          ...
          <div className="flex justify-between w-full">
            <Input value={title} disableUnderline placeholder="Title" className="w-full bg-white px-2 py-1 rounded-xl mr-2" name="title" onChange={(e) => setTitle(e.target.value)}></Input>
            <Button disabled={!title || !sentence} className="w-6/12 !bg-green-400 hover:bg-green-500 disabled:opacity-50 !disabled:bg-green-400" variant="contained" size="medium" type="submit">Submit</Button>
          </div>
        </form>
      }
      {results.length > 0 && (
          ...
          {results.map((result, i) => (
            <div key={i} className="flex flex-row items-center">
              <div className="text-center text-xl m">
                {result.title}:{" "}
                <a href={result.url} target="_blank" className={`text-center ${result.url === "No songs found" ? "text-red-400" : "text-green-400"} mb-2`}>
                  {result.url}
                </a>
              </div>
            </div> 
